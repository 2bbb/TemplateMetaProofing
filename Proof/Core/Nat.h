...
namespace Nat {
    ...
    template <typename M, typename = Enable<AreNats<M>>>
    struct Suc : public NatType {};
    
    template <typename M, typename N, typename = Enable<AreNats<M, N>>>
...
namespace Axiom {
    ...
    public:
        // m = n -> s(m) = s(n)
        template <typename M, typename N, typename ... As, typename = Enable<Nat::AreNats<M, N>>>
        ...
        // add(m, 0) = m
        template <typename M, typename = Enable<Nat::AreNats<M>>>
        ...
        // add(m, s(n)) = s(add(m, n))
        template <typename M, typename N, typename = Enable<Nat::AreNats<M, N>>>
        ...
        // induction
        // P(0) => P(m) => P(s(m)) => ∀m.P(m)
        template <typename X, typename Prop, typename K, typename ... As, typename ... Bs, typename = Enable<Nat::AreNats<X, K>>>
